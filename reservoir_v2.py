...
class Reservoir:
    ...
    def draw(self, partial=False):
        ...
        if not partial:
            ...
            plt.figure()
            plt.plot(self.dataset_out[0], self.dataset_out[1], label='Simulation')
            plt.plot(self.S[0], self.S[1], label='Reservoir')
            plt.figure()
            plt.plot(self.dataset_out[0]-self.S[0], self.dataset_out[1]-self.S[1], label='error')
            plt.show()
        else:
            plt.subplots(self.P, 1)
            # plt.suptitle(self.model_name +', N = ' + str(self.N) + ', Degree = %.5f' % (self.D))
